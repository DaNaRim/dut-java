package dataStructures.lesson5;

import dataStructures.StdDraw;

import java.awt.*;
import java.util.Arrays;

public class Demo2 {

    private static final long[] nArray = {1000, 10_000, 100_000, 1_000_000, 3_000_000, 5_000_000,
            7_000_000, 10_000_000, 15_000_000, 20_000_000, 25_000_000};

    public static void main(String[] args) throws InterruptedException {

        test(new ForLoop(), nArray, Color.RED);
        test(new DoubleForLoop(), Arrays.stream(nArray).map(i -> i / 1000).toArray(), Color.BLUE);
        Thread.sleep(4000);

        test(new ForLoopWithVariableLimits(), nArray, Color.GREEN);
        Thread.sleep(4000);

        test(new ForLoopWithVariableLimits2(), nArray, Color.CYAN);
        test(new DoubleForLoopWithPermanentLoop(), Arrays.stream(nArray).map(i -> i / 1000).toArray(), Color.MAGENTA);
        test(new DoubleForLoopWithVariableLoop(), Arrays.stream(nArray).map(i -> i / 1000).toArray(), Color.ORANGE);

        System.out.println("Complete");
    }

    public static void test(Method method, long[] array, Color color) {
        long[] times = testPerformance(method, array);
        printGraph(array, times, color, method.getClass().getSimpleName());
    }

    private static long[] testPerformance(Method method, long[] array) {
        StopWatch sw = new StopWatch();

        long[] timeArr = new long[array.length];

        for (int i = 0; i < array.length; i++) {
            long n = array[i];

            sw.start();
            method.run(n);
            sw.stop();

            System.out.printf("%s: n = %d time = %d nanos%n", method.getClass().getSimpleName(), n, sw.getTime());
            timeArr[i] = sw.getTime();
        }
        return timeArr;
    }

    private static void printGraph(long[] nArr, long[] timeArr, Color color, String methodName) {
        StdDraw.clear();
        prepareBoard(timeArr, nArr, methodName);

        StdDraw.setPenRadius(0.008);
        StdDraw.setPenColor(color);

        for (int i = 0; i < timeArr.length; i++) {
            if (timeArr[i] == 0) continue;
            StdDraw.point(nArr[i], timeArr[i]);
        }
    }

    private static void prepareBoard(long[] times, long[] arr, String methodName) {
        final long maxTime = Arrays.stream(times).max().getAsLong();
        final long maxN = Arrays.stream(arr).max().getAsLong();

        final long borderX = Math.round(maxN * 0.15);
        final long borderY = Math.round(maxTime * 0.1);

        StdDraw.setXscale(-borderX, maxN + borderX);
        StdDraw.setYscale(-borderY, maxTime + borderY);

        printCoordinateLines(maxN, maxTime, methodName);
    }

    private static void printCoordinateLines(long arrayLength, long maxTime, String methodName) {
        StdDraw.setPenColor(StdDraw.BLACK);
        StdDraw.setPenRadius(0.001);
        StdDraw.setFont(new Font("Arial", Font.PLAIN, 10));

        final int lineIndentOy = (int) Math.round(arrayLength * 0.04) + 1;
        final int lineIndentOx = (int) Math.round(maxTime * 0.04) + 1;

        final long arrayHalf = arrayLength / 2;
        final long timeHalf = Math.round(((maxTime - maxTime * 0.01) / 2) / 100) * 100; //round to hundreds
        final long timeMax = Math.round((maxTime - maxTime * 0.01) / 100) * 100; //round to hundreds

        StdDraw.line(-lineIndentOy, 0, arrayLength, 0); //Ox
        StdDraw.line(0, -lineIndentOx, 0, maxTime); //Oy

        StdDraw.text(-lineIndentOy, -lineIndentOx, "0");

        //Oy
        StdDraw.text(-lineIndentOy, 1, "1");
        StdDraw.text(-lineIndentOy, timeHalf, Long.toString(timeHalf));
        StdDraw.text(-lineIndentOy, timeMax, Long.toString(timeMax));

        //Ox
        StdDraw.text(1, -lineIndentOx, "1");
        StdDraw.text(arrayHalf, -lineIndentOx, Long.toString(arrayHalf));
        StdDraw.text(arrayLength, -lineIndentOx, Long.toString(arrayLength));

        StdDraw.setFont(new Font("Arial", Font.PLAIN, 20));
        StdDraw.text(arrayHalf, timeMax + lineIndentOx * 2L, methodName);
    }

    private interface Method {
        void run(long n);
    }

    private static class ForLoop implements Method {
        @Override
        public void run(long n) {
            for (int j = 0; j < n; j++) {
                int a = (int) (Math.random() * 10);
                int b = (int) (Math.random() * 10);
            }
        }
    }

    private static class DoubleForLoop implements Method {
        @Override
        public void run(long n) {
            for (int j = 0; j < n; j++) {
                for (int k = 0; k < n; k++) {
                    int a = (int) (Math.random() * 10);
                    int b = (int) (Math.random() * 10);
                }
            }
        }
    }

    private static class ForLoopWithVariableLimits implements Method {
        @Override
        public void run(long n) {
            for (long j = n; j > 0; j /= 2) {
                int a = (int) (Math.random() * 10);
                int b = (int) (Math.random() * 10);
            }
        }
    }

    private static class ForLoopWithVariableLimits2 implements Method {
        @Override
        public void run(long n) {
            for (long j = 1; j <= n; j *= 2) {
                int a = (int) (Math.random() * 10);
                int b = (int) (Math.random() * 10);
            }
        }
    }
    
    private static class DoubleForLoopWithPermanentLoop implements Method {
        @Override
        public void run(long n) {
            for (int j = 0; j < n; j++) {
                for (int k = 0; k < 10_000; k++) {
                    int a = (int) (Math.random() * 10);
                    int b = (int) (Math.random() * 10);
                }
            }
        }
    }

    private static class DoubleForLoopWithVariableLoop implements Method {
        @Override
        public void run(long n) {
            for (int j = 0; j < n; j++) {
                for (int k = 0; k < j; k++) {
                    int a = (int) (Math.random() * 10);
                    int b = (int) (Math.random() * 10);
                }
            }
        }
    }

}
